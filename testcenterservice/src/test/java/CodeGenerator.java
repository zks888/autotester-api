...
class CodeGenerator {
    // JDBC配置，请修改为你项目的实际配置
    private static final String JDBC_URL = "jdbc:mysql://127.0.0.1:3306/testcenter?useUnicode=true&useSSL=false&allowMultiQueries=true&characterEncoding=utf-8&useLegacyDatetimeCode=false&serverTimezone=UTC";
    ...
    // 模板位置
    private static final String TEMPLATE_FILE_PATH = CodeGenerator.PROJECT_PATH + "/testcenterservice/src/test/resources/generator/template";
    // java文件路径
    ...
    // 生成的Service存放路径
    private static final String PACKAGE_PATH_SERVICE = CodeGenerator.packageConvertPath(SERVICE_PACKAGE);
    // 生成的Service实现存放路径
    private static final String PACKAGE_PATH_SERVICE_IMPL = CodeGenerator.packageConvertPath(SERVICE_IMPL_PACKAGE);
    // 生成的Controller存放路径
    private static final String PACKAGE_PATH_CONTROLLER = CodeGenerator.packageConvertPath(CONTROLLER_PACKAGE);
    // @author
    private static final String AUTHOR = "zks888";
    // @date
...
//    }
        ...
        CodeGenerator.genCode("process_testcase");
    ...
    private static void genModelAndMapper(final String tableName, String modelName) {
        ...
        javaModelGeneratorConfiguration.setTargetProject(CodeGenerator.PROJECT_PATH + CodeGenerator.JAVA_PATH);
        ...
        sqlMapGeneratorConfiguration.setTargetProject(CodeGenerator.PROJECT_PATH + CodeGenerator.RESOURCES_PATH);
        ...
        javaClientGeneratorConfiguration.setTargetProject(CodeGenerator.PROJECT_PATH + CodeGenerator.JAVA_PATH);
    ...
    private static void genService(final String tableName, final String modelName) {
        try {
            ...
            final String modelNameUpperCamel = StringUtils.isEmpty(modelName)
            ...
            final File file = CodeGenerator.createFileDir(CodeGenerator.PROJECT_PATH
            ...
            final File file1 = CodeGenerator.createFileDir(CodeGenerator.PROJECT_PATH
                    + CodeGenerator.JAVA_PATH
    ...
    private static void genController(final String tableName, final String modelName) {
        try {
            ...
            final String modelNameUpperCamel = StringUtils.isEmpty(modelName)
                    ? CodeGenerator.tableNameConvertUpperCamel(tableName)
                    : modelName;
            data.put("baseRequestMapping", CodeGenerator.modelNameConvertMappingPath(modelNameUpperCamel));
            data.put("modelNameUpperCamel", modelNameUpperCamel);
